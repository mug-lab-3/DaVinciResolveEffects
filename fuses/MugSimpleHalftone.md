...
小さなドットの境界を滑らかにするため、距離ベースのアンチエイリアシングを実装しています。

```mermaid
graph LR
    A[ピクセル-ドット中心距離] --> B{dist < innerRadius?}
    B -->|Yes| C[完全不透明<br/>coverage = 1.0]
    B -->|No| D{dist < dotRadius?}
    D -->|Yes| E[グラデーション計算<br/>coverage = smoothstep]
    D -->|No| F[完全透明<br/>coverage = 0.0]
