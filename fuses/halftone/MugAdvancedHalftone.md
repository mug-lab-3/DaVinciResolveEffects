...
`MugAdvancedHalftone.fuse` は、DaVinci Resolve/Fusion 向けの高品質なリアルタイム・ハーフトーン生成プラグイン (v1.70) です。
...
- **アナログ印刷のシミュレート（Jitter）**: 擬似乱数を用いた位置ズレ（Position Jitter）、サイズムラ（Size Jitter）、縦横比のブレ（Aspect Jitter）による有機的でアナログな不完全さの再現。滑らかなアニメーションに対応する連動ノイズ（Jitter Noise Phase）を実装。
...
---
*Generated based on source code MugAdvancedHalftone.fuse v1.70*
