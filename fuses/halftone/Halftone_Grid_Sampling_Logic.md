...
`MugAdvancedHalftone.fuse` は、入力された画像を無数の小さなマス目（セル）に分割し、それぞれのマス目の明るさに応じてドットを描画します。

このドキュメントでは、画像をどのように分割して「マス目（セル）」を定義しているのか、そして各マス目が元画像の明るさ（輝度）をどのように読み取って（サンプリングして）いるのか、その**最適化されたグリッド生成とサンプリングロジック**について解説します。
...
### CPU側での事前計算（メモリ確保）
GPUに計算を投げる前、CPUのLuaコード上でまず画面サイズと密度からピッチを計算し、`cellGridNumX` と `cellGridNumY`（画面内にマス目が何個あるか）を割り出します。これは**「中間データを保存するテクスチャバッファ（`dst_info1` 等）を、ぴったりマス目の数と同じ解像度で無駄なくメモリ確保する」**という重要な役割を担っています。
...
`MugAdvancedHalftone.fuse` では、第1パス（`CellInfoKernel`）で計算した「マス目の中心座標」や「ドット半径」といった数値を、一時的な画像データ（テクスチャバッファ `dst_info1`, `dst_info2` 等）に書き込み、第2パスでそれを読み取っています。このように**「画像を計算結果を保存する『配列・メモリ』として扱う」**場合、GPUに対しテクスチャの読み書き方法（サンプラー設定）を厳密に指定しなければ、データが勝手に改変されてしまいます。

`Process` 関数内（1195行目付近）では、以下の必須設定が記述されています。
