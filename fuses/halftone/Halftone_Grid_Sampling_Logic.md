...
`MugAdvancedHalftone.fuse` では、第1パス（`CellInfoKernel`）で計算した「マス目の中心座標」や「ドット半径」といった数値を、一時的な画像データ（テクスチャバッファ `dst_info1`, `dst_info2` 等）に書き込み、第2パスでそれを読み取っています。
...
これを防ぐため、読込時の設定（サンプラー）を通じて「絶対に値を混ぜるな」とGPUに命令を出す必要があります。`Process` 関数内（1195行目付近）では、以下の必須設定が記述されています。
